...
function getAllFiles(dirPath: string, files: string[] = []): string[] {
  ...
  items.forEach((item) => {
    ...
    if (item === ".git" || item === ".gitignore") return;
 ...
 */
function fixHtmlPaths(htmlContent: string): string {
  // Fix src attributes: src="/file" -> src="file"
  htmlContent = htmlContent.replace(
    /(<(?:script|img|iframe)[^>]+src=["'])\/(?!\/)/gi,
    "$1",
  );
 ...
 */
export async function uploadDirectoryToBlob(
  localPath: string,
  folderName: string,
): Promise<string> {
  try {
 ...
 */
export function deleteLocalDirectory(localPath: string): void {
  try {
